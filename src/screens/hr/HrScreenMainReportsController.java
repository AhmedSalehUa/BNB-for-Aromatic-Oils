...
public class HrScreenMainReportsController implements Initializable {
 Preferences prefs;
     ...
     */
    @Override
    public void initialize(URL url, ResourceBundle rb) { prefs = Preferences.userNodeForPackage(BNBForAromaticOils.class);
    ...
    private void allSalaryxReport(ActionEvent event) {
        try {
            ...
            Stage st = new Stage();
            st.getIcons().add(new Image(getClass().getResourceAsStream("/assets/icons/logo.png")));
            st.setTitle("BNB For Aromatic Oils (تقرير الحضور والانصراف)");
            st.setScene(sc);
            Rectangle2D screenBounds = Screen.getPrimary().getVisualBounds();
            st.setX((screenBounds.getWidth() - 1360) / 2);
            st.setY((screenBounds.getHeight() - 760) / 2);
    ...
    private void clacAttend(ActionEvent event) {
         try {
            ...
            Stage st = new Stage();
            st.getIcons().add(new Image(getClass().getResourceAsStream("/assets/icons/logo.png")));
            st.setTitle("BNB For Aromatic Oils (حساب الحضور والانصراف)");
            st.setScene(sc);
            Rectangle2D screenBounds = Screen.getPrimary().getVisualBounds();
            st.setX((screenBounds.getWidth() - 1360) / 2);
            st.setY((screenBounds.getHeight() - 760) / 2);
